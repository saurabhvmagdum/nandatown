...
<!-- Nanda Town Hackathon Judge Rubric -->

version: 1

# Nanda Town Hackathon Judge Rubric

You are one of several independent judges for the Nanda Town hackathon. Nanda Town is a
...
### 3. api_fit — drop-in compatibility, registry wiring, idiomatic to Nanda Town
...
- **1** — Pure textbook restatement with no insight beyond the Wikipedia
  page. Or: duplicates an existing Nanda Town reference plugin without
  acknowledging it.
- **3** — Standard algorithm well-applied to the Nanda Town context, with at
  least one judgment call that shows the author understood the domain
  (e.g., a sensible default, a Nanda Town-specific adaptation, a validator that
  isn't obvious).

...
def validate_no_conflicting_commits(events: list[dict[str, Any]]) -> list[ValidationResult]:
    ...
    if not has_commits:
        return [
            ValidationResult("validate_no_conflicting_commits", False, "No commit is observable.")
        ]
        ...
        if len(digests) > 1:
            return [
                ValidationResult(
                    ...
                    False,
                    f"Conflicting commits at height {height}: {digests}",
                )
            ]

    return [
        ValidationResult("validate_no_conflicting_commits", True, "No conflicting commits found.")
    ]
...
def validate_no_equivocation_in_certificate(events: list[dict[str, Any]]) -> list[ValidationResult]:
    ...
    for ev in events:
        ...
        if msg.startswith("equivocation:"):
            ...
            if "height" in parts and "round" in parts and "agent" in parts:
                equivocators_by_round[(int(parts["height"]), int(parts["round"]))].add(
                    parts["agent"]
                )
            ...
            if (
                "height" not in parts
                or "round" not in parts
                or "qc" not in parts
                or "signers" not in parts
            ):
                return [
                    ValidationResult(
                        "validate_no_equivocation_in_certificate",
                        False,
                        "Certificate malformed or missing.",
                    )
                ]
            ...
            if len(signers) != len(set(signers)):
                return [
                    ValidationResult(
                        "validate_no_equivocation_in_certificate",
                        False,
                        "Certificate contains duplicate signers.",
                    )
                ]
                ...
                if signer in excluded_actual:
                    return [
                        ValidationResult(
                            "validate_no_equivocation_in_certificate",
                            False,
                            f"Signer {signer} was excluded but counted in certificate.",
                        )
                    ]
    ...
    if not has_commits:
        return [
            ValidationResult(
                "validate_no_equivocation_in_certificate",
                False,
                "No commits found to validate certificate.",
            )
        ]
    ...
    return [
        ValidationResult(
            "validate_no_equivocation_in_certificate",
            True,
            "All certificates valid and equivocators excluded.",
        )
    ]
...
def validate_no_forged_quorum(events: list[dict[str, Any]]) -> list[ValidationResult]:
    ...
    commits_to_check: list[dict[str, Any]] = []
    ...
    for ev in events:
        ...
        if msg.startswith("vote:"):
            ...
            if "height" in parts and "round" in parts and "digest" in parts and "agent" in parts:
                ...
                votes_by_round_digest[
                    (int(parts["height"]), int(parts["round"]), parts["digest"])
                ].add(parts["agent"])
        ...
        if len(actual_votes) < threshold:
            return [
                ValidationResult(
                    ...
                    False,
                    f"Commit forged: has {len(actual_votes)} votes, needs {threshold}.",
                )
            ]

    return [
        ValidationResult(
            "validate_no_forged_quorum", True, "All commits backed by valid 2f+1 quorums."
        )
    ]
...
def validate_no_stuck_view(events: list[dict[str, Any]]) -> list[ValidationResult]:
    ...
    for ev in events:
        ...
        if msg.startswith("propose:"):
            ...
            view_changes += 1
        elif msg.startswith("commit:") and view_changes > 0:
    ...
    if view_changes > 0 and commits_after_view_change == 0:
        return [
            ValidationResult(
                ...
                False,
                "View changed but no subsequent commit occurred (stuck view).",
            )
        ]

    return [
        ValidationResult("validate_no_stuck_view", True, "Progress maintained across view changes.")
    ]

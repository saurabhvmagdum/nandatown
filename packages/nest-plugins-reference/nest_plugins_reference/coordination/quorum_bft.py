...
class QuorumBFT:
    ...
    # -----------------------------------------------------------------------
    def sign_message(
        self, message_type: str, fields: dict[str, str], identity_plugin: Any
    ) -> bytes:
    ...
    def process_vote(
        ...
        if existing_digest is not None and existing_digest != digest:
            ...
            msg = f"equivocation:height={height}|round={round_id}|agent={voter}|vote_a={existing_digest}|vote_b={digest}|evidence={evidence}"  # noqa: E501
            return False, msg
    ...
    def check_quorum(
        self, height: int, round_id: int, digest: str, total_nodes: int
    ) -> list[str] | None:
    ...
    def generate_certificate(
        self, height: int, round_id: int, digest: str, signers: list[str]
    ) -> str:
        ...
        cert_data = f"height={height}|round={round_id}|digest={digest}|signers={','.join(sorted(signers))}|excluded={','.join(excluded)}"  # noqa: E501

...
from nest_core.cli import _run_scenario
...
def _load_trace(path: Path | str) -> list[dict]:
    with open(path) as f:
        return [json.loads(line) for line in f if line.strip()]

def _message_body(ev: dict) -> str:
...
async def test_byzantine_scenario_passes_validators() -> None:
    ...
    equivocation_events = [ev for ev in events if ev.get("kind") == "send" and _message_body(ev).startswith("equivocation:")]
    ...
    commit_events = [ev for ev in events if ev.get("kind") == "send" and _message_body(ev).startswith("commit:")]
    ...
    for ev in commit_events:
        ...
        assert len(set(signers)) >= 5, f"Commit requires at least 5 non-excluded signers. Got {len(set(signers))}"
